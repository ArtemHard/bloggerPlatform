...
describe('Blog Posts With Likes API', () => {
  const app = express();
  setupApp(app);
  ...
  it('should return blog posts with correct likes info for user 1; GET /blogs/:blogId/posts', async () => {
    ...
    // Проверяем что все посты принадлежат правильному блогу
  ...
  it('should return blog posts with pagination; GET /blogs/:blogId/posts', async () => {
    // Тест пагинации
  ...
  it('should return blog posts with sorting by createdAt; GET /blogs/:blogId/posts', async () => {
    const response = await userAuth(userTokens[0])
      .get(`${BLOGS_PATH}/${blogId}/posts`)
  ...
  it('should return blog posts with correct likes info for different users; GET /blogs/:blogId/posts', async () => {
  ...
  it('should return 404 for non-existent blog; GET /blogs/:blogId/posts', async () => {
    const nonExistentBlogId = '6507a1b1b1a1b1a1b1a1b1a1';

    await userAuth(userTokens[0])
      .get(`${BLOGS_PATH}/${nonExistentBlogId}/posts`)
      .expect(HttpStatus.NotFound);
  });

  it('should return empty posts list for blog with no posts; GET /blogs/:blogId/posts', async () => {
    // Создаем пустой блог
    const emptyBlogResponse = await auth()
      .post(BLOGS_PATH)
      .send({
        name: 'Empty Blog',
    ...
    const emptyBlogId = emptyBlogResponse.body.id;

    const response = await userAuth(userTokens[0])
      .get(`${BLOGS_PATH}/${emptyBlogId}/posts`)
      .expect(HttpStatus.Ok);

    const postsData = response.body;
    expect(postsData.pagesCount).toBe(0);
    expect(postsData.page).toBe(1);
    expect(postsData.totalCount).toBe(0);
